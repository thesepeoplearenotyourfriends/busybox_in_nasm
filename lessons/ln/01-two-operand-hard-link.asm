; ln.asm - teaching implementation of a small `ln` utility.
;
; Behavior implemented:
;   - Accept exactly two operands: TARGET LINK_NAME.
;   - Create a hard link named LINK_NAME that points at the same inode as TARGET.
;
; Behavior missing:
;   - Symbolic links (`-s`), force/interactive options, directory handling,
;     target-directory options, `--help`, and `--version` are not implemented.
;
; Syscalls used:
;   - link(2), write(2), exit(2)
;
; Teaching focus:
;   - A hard link is made by asking the kernel to add another directory entry
;     for an existing inode; no file bytes are copied.
...
section .rodata
missing_msg:    db "ln: missing operand", 10, 0
unexpected_msg: db "ln: unexpected extra operand: ", 0
fail_msg:       db "ln: failed to create hard link ", 0
arrow:          db " -> ", 0
newline:        db 10
...
_start:
    ...
    ; arg1 rdi = existing target path; arg2 rsi = new hard-link pathname.
    mov rdi, [r13 + 8]
    mov rsi, [r13 + 16]
