...
- **Implemented behavior:** `cmp [-l] [-s] [-n LIMIT] [--] FILE1 [FILE2 [SKIP1 [SKIP2]]]`. `FILE2` defaults to stdin and either input may be `-`. Default output reports the first unequal one-based character and line; `-l` reports every unequal byte with unpadded one-based decimal positions and three-column octal values; `-s` suppresses difference and error output; `-n` compares at most `LIMIT` bytes after the skips. Short options may be clustered, except that `-l` and `-s` are incompatible and fail with status `2`.
...
- **Stream and buffer policy:** each distinct input owns an independent 4096-byte buffer and cursor. The comparison never assumes that parallel reads return equal-sized chunks, handles binary NUL bytes, retries interrupted reads, and has no input-size truncation boundary. If both operands are `-`, they name the same stdin stream and compare equal without consuming it, following GNU and BusyBox practice.
...
- **Syscalls used:** `open(2)`, `read(2)`, `write(2)`, `close(2)`, and `exit(2)`.
- **Reference:** the supported output, option, stdin, skip, EOF, and status surface is differentially tested in the C locale against GNU diffutils 3.10. GNU's size-derived leading padding on `-l` byte positions is normalized because this streaming implementation intentionally prints unpadded positions. Numeric-suffix behavior remains outside that comparison surface.
